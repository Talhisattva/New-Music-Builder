...
def test_write_export_scaffold_creates_expected_files(tmp_path: Path) -> None:
    ...
    assert 'description=[td][i]MADE FOR:[/i] [b]NEW MUSIC [42+][/b][/td]' in workshop_text
    assert 'description=[td][i]GENERATED BY:[/i] [b]NEW MUSIC BUILDER[/b][/td]' in workshop_text
...
def test_write_export_scaffold_uses_singles_legend_and_section_labels_for_singles_rows(tmp_path: Path) -> None:
    ...
    row.tracks_a = [
        ...
        TrackEntry(source_path='C:/a2.ogg', display_label='Song Two', duration='00:02:00'),
    ]
    catalog = AssetCatalog(ASSETS_ROOT).scan()
    plan = build_export_plan(project, catalog)
    targets = resolve_export_target(plan, project.workshop_output_folder, mod_name=project.mod_name, mod_id=project.mod_id)
