...
class BuildExportModule(ModulePanel):
    def __init__(self, master, session, on_build, on_reset):
        super().__init__(master, t('PHASE 3 : BUILD & EXPORT'), header_button=True, command=on_build)
    ...
    def _build(self) -> None:
        ...
        make_builder_label(left, t('QUEUE / PROGRESS'), text_color=theme.TEXT, size=12, weight='bold').pack(anchor='w')
        self.queue_frame.pack(in_=left, fill='both', expand=True, pady=(6, 0))

        make_builder_label(right, t('GENERATED ITEM PREVIEW'), text_color=theme.TEXT, size=12, weight='bold').pack(anchor='w')
        self.preview_frame.pack(in_=right, fill='both', expand=True, pady=(6, 0))

        make_builder_label(self.body, t('BUILD LOG'), text_color=theme.TEXT, size=12, weight='bold').pack(anchor='w', padx=10)
        self.log_box.pack(fill='x', padx=8, pady=(4, 4))

        actions = ctk.CTkFrame(self.body, fg_color='transparent')
        actions.pack(fill='x', padx=8, pady=(0, 8))
        make_builder_button(actions, t('OPEN OUTPUT FOLDER'), lambda: None).pack(side='left', padx=(0, 6))
        make_builder_button(actions, t('CANCEL / RESET'), self.on_reset, variant='secondary').pack(side='left')
        self.refresh([], [])
    ...
    def refresh(self, build_log: list[str], preview_entries: list[str]) -> None:
        ...
        for row in self.session.project.media_rows:
            for side_name, tracks in [('A', row.tracks_a), ('B', row.tracks_b)]:
                ...
                make_builder_label(outer, f'{row.media_name} ({t("Side")} {side_name})', size=12, weight='bold').pack(anchor='w', padx=8, pady=(6, 2))
                progress = ctk.CTkProgressBar(outer)
                apply_builder_progress_style(progress)
                progress.set(0)
                progress.pack(fill='x', padx=8, pady=(0, 4))
                make_builder_label(outer, f'{len(tracks)} {t("songs queued")}', text_color=theme.MUTED, size=11).pack(anchor='w', padx=8, pady=(0, 6))
        ...
        self.log_box.insert('end', '\n'.join(build_log) if build_log else t('Build has not started yet.'))
